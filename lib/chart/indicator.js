  ...
  renderContent() {
    ...
    function renderRow(item, index) {
      ...
      if (item[dimensionKey]) {
        content = `<div class="dimension"><span class="key">${colByName[dimensionKey].comments}</span><span class="value">${item[dimensionKey]}</span></div><div class="measure-wrapper">`;
        ...
        content = `<div class="main-measure"><span class="key">${colByName[first].comments}</span><span class="value">${item[first]}</span></div><div class="measure-wrapper">`;
        lang.forIn(item, (value, key) => {
          if (key !== first) {
            const name = colByName[key].comments;
            content += `<div class="measure"><span class="key">${name}</span> <span class="value">${value}</span></div>`;            
