  ...
  configMeasures(type, measures, dataSet, extraSeriesOption) {
    ...
    _.each(measures, (measure) => {
      ...
      if (!_.isEmpty(measureSchema.format)) {
        data = data.map((v) => Number.parseFloat(v.match(/[0-9.]/g).join('')));
        const formatter = util.reverseFormatter(measureSchema.format);
        if (formatter !== '{value}') {
          const rightValues = this._option['data-valuesRight'] || [];
          if (rightValues.some(v => v === measure)) {
            option.yAxis[1].axisLabel = _.merge(option.yAxis[1].axisLabel, {
              formatter,
            });
          }
          if (this._option['data-values'].some(v => v === measure)) {
            option.yAxis[0].axisLabel = _.merge(option.yAxis[0].axisLabel, {
              formatter,
            });
          }
        }
