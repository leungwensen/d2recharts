  ...
  configMeasures(type, measures, dataSet, extraSeriesOption) {
    ...
    _.each(measures, (measure) => {
      ...
      if (!_.isEmpty(measureSchema.format)) {
        ...
        if (formatter !== '{value}') {
          const idx = this._option['data-valuesRight'] && this._option['data-valuesRight'].some(v => v === measure) ? 1 : 0;
          option.yAxis[idx].axisLabel = _.merge(option.yAxis[idx].axisLabel, {
            formatter,
          });
        }
      }
